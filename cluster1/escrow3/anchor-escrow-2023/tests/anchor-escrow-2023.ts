...
import tak from "./taker_wallet.json"

// Import keypair from the wallet file
const keypair = Keypair.fromSecretKey(new Uint8Array(wallet));
const takerKeypair = Keypair.fromSecretKey(new Uint8Array(tak));
...
describe("anchor-escrow-2023", () => {
  ...
  const programId = new PublicKey("E8YJXy9iYuKYK9GDn8GJoPyKHCJR4eT3jWAyGq6RArXc");
  const program = new anchor.Program<AnchorEscrow2023>(IDL, programId, anchor.getProvider());

  // Set up our keys
  const [maker, taker] = [keypair, takerKeypair];
